...
- [ ] T001 Add `.github/workflows/test.yml`: Node 22, `npm install`, `npm test` on
      push and pull_request (package-lock is gitignored, so no `npm ci`).
- [ ] T002 [P] Add `plugins/quartz-okf/test/` to the `test` script in `package.json`
      (`node --test core/test/*.test.js harness/test/*.test.js plugins/quartz-okf/test/*.test.js`).
- [ ] T003 [P] Document the additive graph fields as "planned" in
...
- [ ] T004 Characterization test in `core/test/graph.test.js`: snapshot the exact
      `edges` array (order included) `buildGraph` produces for a fixture with
      declared, mirrored and unmirrored labels.
- [ ] T005 Extract `deriveInverseEdges(edges, profile)` into `core/lib/graph.js`,
      call it from `buildGraph`, export it from `core/lib/index.js`; T004 stays green
      unchanged.
- [ ] T006 Test in `plugins/quartz-okf/test/emitter.test.js`: with
...
- [ ] T007 Implement `baseUrl` in `emitAll` (`plugins/quartz-okf/dist/index.js`):
...
- [ ] T008 [P] [US1] `core/test/federation.test.js` — `validateFederationConfig`:
...
- [ ] T009 [P] [US1] `federateGraph` — open filter: only nodes whose
...
- [ ] T010 [P] [US1] `federateGraph` — portal marker: `subgraph{id,title,site,graph,
      source_head,notes,previewed}`; `stats.federatedNodes/federatedEdges`; existing
      `stats` values unchanged.
- [ ] T011 [P] [US1] `federateGraph` — edges: portal→federated declared with the
...
- [ ] T012 [P] [US1] `federateGraph` — post-fetch problems: `site-required` when
...
- [ ] T013 [P] [US1] `absolutiseChildGraph` — every node gets `url: <site>/<slug>`
...
- [ ] T014 [P] [US1] `plugins/quartz-okf/test/emitter.test.js` — with a `federation`
...
- [ ] T015 [US1] Create `core/lib/federation.js` with `validateFederationConfig`,
      `federateGraph`, `absolutiseChildGraph`; export all from `core/lib/index.js`.
- [ ] T016 [US1] Emitter shell in `plugins/quartz-okf/dist/index.js`: `federation`
...
- [ ] T017 [US1] Update `plugins/quartz-okf/README.md` § Graph shape and add a
...
- [ ] T018 [US2] `indexar()`: carry `subgraph`, `federated` and the root
      `federatedFrom` into the internal model.
- [ ] T019 [US2] Drawing: nodes with `subgraph` go to the `sueltos` path with a double
...
- [ ] T020 [US2] Navigation stack: "Explore subgraph" button in the reading-panel
...
- [ ] T021 [US2] Deep link: `?graph=<id>` loads `/static/okf-subgraphs/<id>.json`
      first and renders a back action from `federatedFrom`.
- [ ] T022 [US2] Search results: badge with the subgraph id on federated nodes;
      `buscarNodo` unchanged (prefixed ids already match).
- [ ] T023 [US2] `plugins/quartz-okf-explorer/README.md`: document portals, dive,
      back and the deep link.
- [ ] T024 [US2] Verify on a consumer build (quickstart §3) and record the walk in
...
- [ ] T025 [P] [US3] `core/test/federation.test.js`: `pin` ≠ child `source_head`
...
- [ ] T026 [P] [US3] `plugins/quartz-okf/test/emitter.test.js`: `fetchBundle`
...
- [ ] T027 [US3] Drift warning in `federateGraph`; unreachable-child handling in the
...
- [ ] T029 Child consumer `cern-it-governance-graph`: bump `okf/quartz-okf.ref`
...
- [ ] T030 New parent consumer `Zetesis-Labs/cern-graph` (public, MIT), created from
...
- [ ] T032 [P] `docs/METHODOLOGY.md`: add the consumer adoption recipe (ref bump →
