...
import { loadConsumerConfig } from "../consumer-config.ts"
import { loadDocuments } from "../files.ts"
import { validateDocuments } from "../rules.ts"
import { buildGraph } from "../graph.ts"
import { RECIPES, RECIPE_BY_TYPE, weaveDiagram } from "../diagram.ts"
...
for (const document of documents) {
  ...
  const recipeName = document.frontmatter?.diagram ?? (type ? RECIPE_BY_TYPE[type] : undefined)
